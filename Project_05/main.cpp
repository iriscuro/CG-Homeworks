...
void updateCirclePosition(std::vector<float>& circleVertices, float& circleX, float& circleY,  float radius, float maxSpeed);
...
float circleSpeedX = 0.000f;  // Velocidad en el eje X
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
		...
		// Update
		//if (isCircleMoving)
			//std::cout<<isCircleMoving<<std::endl;
            updateCirclePosition(circleVertices, circleX, circleY, circleRadius);
        
		//updateCirclePosition(circleVertices, circleX, circleY, circleRadius);
		//updateCirclePosition(circleVertices, circleX, circleY, circleRadius, maxSpeed);
...
void updateCirclePosition(std::vector<float>& circleVertices, float& circleX, float& circleY, float radius, float maxSpeed) {
    static int direction = 0; // Variable para rastrear la dirección del movimiento
    static float targetSpeedX = 0.0f;
    static float targetSpeedY = 0.0f;

    // Determina las velocidades objetivo en función de la dirección actual
    switch (direction) {
        case 0: // Mover hacia arriba
            targetSpeedX = 0.0f;
    ...
    // Interpola suavemente la velocidad actual hacia las velocidades objetivo
    float interpolationFactor = 0.02f; // Ajusta esto para controlar la suavidad del cambio
    circleSpeedX = (1.0f - interpolationFactor) * circleSpeedX + interpolationFactor * targetSpeedX;
    circleSpeedY = (1.0f - interpolationFactor) * circleSpeedY + interpolationFactor * targetSpeedY;

    // Actualiza la posición del círculo en función de la velocidad actual
    circleX += circleSpeedX;
    circleY += circleSpeedY;
